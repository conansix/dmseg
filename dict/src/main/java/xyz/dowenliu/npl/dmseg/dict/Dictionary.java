 ...
 * @since 1.0.0
 */
public interface Dictionary<T extends WordPath> extends Branching<Character, T>, Iterable<T> {
    /**
     * 字典中的词数
     *
     * @return 收录的词数
    ...
    /**
     * 从字典中删除词。将自动删除词路径中不可达的父路径。
     *
     * @param word 要删除的词。
     * @return 成功删除则返回 {@code true}，否则返回 {@code false}，
     * 可能是字典中不包含此词（使用{@link #contains(String)}判断），
     * 也可能是实现类中执行过程中发生了可忽略的非致命错误。
     */
     ...
     */
    Collection<WordPath> withPrefix(WordPath<T> path);
     ...
     * @return 缓冲区第一个字开头的词
     */
    List<Word> dictMatch(CharBuffer charBuffer);
